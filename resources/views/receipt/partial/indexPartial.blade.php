...
<div uk-height-viewport="offset-top: true; offset-bottom: 50px">
    ...
    <div class="uk-margin-large uk-child-width-1-2@m" uk-grid>
        ...
        @if ($data['setupList']['receipt']['cashTotal'] > 0)
            
            <div class="uk-margin-remove-top">Cash {{$currency}}
                <span class="uk-text-danger" uk-icon="close" onclick="showSetupList('cash')"></span>
            </div>
            <div class="uk-text-right uk-margin-remove-top">{{CurrencyHelper::Format($data['setupList']['receipt']['cashTotal'])}}</div>
        ...
        <div class="uk-text-right uk-margin-remove-top uk-text-bold">{{CurrencyHelper::Format($data['setupList']['receipt']['priceFinalTotal'])}}</div>
