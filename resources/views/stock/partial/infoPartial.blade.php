        ...
        <div class="uk-card uk-card-default uk-padding">
            ...
            <div class="uk-margin">
                ...
                <select class="uk-select" name="stock_group_id">
                    ...
                    @if ($data['settingModel']->setting_stock_group)
                        @foreach (explode(",", $data['settingModel']->setting_stock_group) as $group)
                            @php
                                $selected = '';
                                if($loop->iteration == $data['stockModel']->stock_group_id){
                                        $selected = 'selected';
                                }
                            @endphp
                            <option value="{{$loop->iteration}}"  {{$selected}}>{{$group}}</option>
                        @endforeach
                    @endif
                </select>
            </div>
            ...
            <div class="uk-margin">
                ...
                <select class="uk-select" name="stock_category_id">
                    ...
                    @if ($data['settingModel']->setting_stock_category)
                        @foreach (explode(",", $data['settingModel']->setting_stock_category) as $category)
                            @php
                                $selected = '';
                                if($loop->iteration == $data['stockModel']->stock_category_id){
                                        $selected = 'selected';
                                }
                            @endphp
                            <option value="{{$loop->iteration}}"  {{$selected}}>{{$category}}</option>
                        @endforeach
                    @endif
                </select>
            </div>
            ...
            <div class="uk-margin">
                ...
                <select class="uk-select" name="stock_vat_id">
                    ...
                    @if ($data['settingModel']->setting_vat)
                        @foreach (explode(",", $data['settingModel']->setting_vat) as $vat)
                            @php
                                $selected = '';
                                if($loop->iteration == $data['stockModel']->stock_vat_id){
                                        $selected = 'selected';
                                }
                            @endphp
                            <option value="{{$loop->iteration}}"  {{$selected}}>{{$vat}}</option>
                        @endforeach
                    @endif
                </select>
            </div>
